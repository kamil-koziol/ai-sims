...
async def add_agent(
    add_agent_request: AddAgentRequest, state: State = Depends(get_state)
):
    ...
    GameManager().games[game_id].add_agent(
        AgentMapper.request_to_agent(add_agent_request.agent)
    )
